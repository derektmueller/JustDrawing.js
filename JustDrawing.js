...
JustDrawing.prototype._setUpCanvas = function () {
    ...
    $(this._parentElement).width (this.width);
...
JustDrawing.prototype._setUpControls = function () {
    ...
    var controls = $('<div>', { 'class': 'jd-controls' }).append (
        ...
        $('<button>', {
            'class': 'jd-source-button',
            'text': 'GitHub'
        }),
        $('<button>', {
            'class': 'jd-save-button',
            'text': 'Save Drawing',
        })
    );
    ...
    $(controls).find ('.jd-save-button').click (function () {
        var surface = that._getSurface ();
        window.open (surface.getTopCanvas ().toDataURL (), '_blank');
    });

    $(controls).find ('.jd-source-button').click (function () {
        window.open ('http://github.com/parenparen', '_blank');
    });
